...
| [oracle-price-reader](examples/oracle-price-reader) | A live Pyth stream for XAU/USD and USDC/CHF, printed with the three things that make a price usable: the feed it came from, the exponent it is scaled by, and the time it was published. USDC/CHF is derived from two feeds because it is not one, and a stale feed is reported apart from a closed market so the alarm keeps meaning something. | TypeScript, Node, Pyth Hermes, SSE | None yet | Spec |
