...
| [stellar-trace](examples/stellar-trace) | Indexes Stellar contract events into SQLite and keeps the raw XDR — a `getEvents` loop that pages by cursor and resumes after a kill with no gap and no duplicates — plus `trace <tx-hash>`, which decodes a transaction's meta into the ledger entries it moved, before and after. | TypeScript, Node, SQLite, `@stellar/stellar-sdk` | Building |
