 ...
 * for the wallet extension — see scripts/local-signer.ts — so everything except
 * the signature itself is the real code. What that leaves untested is exactly
 * what a wallet is responsible for: the approval, and the key.
 *
...
const { localSigner } = await import("./local-signer.js");
type Failure = import("../lib/stellar/errors.js").ContributionFailure;
type Signer = import("../lib/signing/signer.js").Signer;

/**
 * A `Signer` for the contribution path with no extension behind it.
 *
 * Built on the far side of the module split described above, and handed a
 * secret rather than a `Keypair` for the same reason. It stands in for the
 * wallet the way a wallet behaves: signed XDR comes back as a new transaction
 * rather than the one that went in.
 */
const signerFor = (kp: Keypair): Signer => localSigner(kp.secret());
...
// --- the return-value hazard ----------------------------------------------
// The property the Signer interface is written around: a wallet hands back a
// different transaction from the one it was given, so the signature is on the
// return value and not on the argument. Every contribute() call above would
// have failed at submission if lib/stellar submitted its argument, but that is
// implicit; this states it.
console.log("\nSigner contract:");
{
  const account = await horizon.loadAccount(donor.publicKey());
  ...
  const signed = await signerFor(donor).signTransaction(built);
  check("the signed transaction is a different object", signed !== built);
  check(
    "and the argument was left unsigned",
    built.signatures.length === 0,
    `argument carries ${built.signatures.length} signature(s)`,
  );
  check("while the return value carries one", signed.signatures.length === 1);
  check(
    "over the same transaction hash",
    signed.hash().toString("hex") === built.hash().toString("hex"),
  );
}
